...
curl --fail --silent --show-error --insecure --head \
  "${BASE_URL}/frozen-vault/static/css/app.css" >/dev/null

printf 'Checking deployment metadata endpoint...\n'
curl --fail --silent --show-error --insecure \
  "${BASE_URL}/frozen-vault/utils/deployment" >/dev/null

printf 'Checking generated HTML for mixed-content local URLs...\n'
html="$(curl --fail --silent --show-error --insecure "${BASE_URL}/frozen-vault/")"
