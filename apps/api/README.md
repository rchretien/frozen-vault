...
From the repository root, serve the backend with:

```sh
uv run poe api --dev
```

The explicit backend-targeting form is:

```sh
uv run --directory apps/api poe api --dev
```

You can also start the containerized backend with:
...
If you are already inside the `apps/api` directory, this is equivalent to:
